...
Use the step function in the GPControls (behind the gear symbol) to slow things down to single steps.  While stopped, you can use the developer tools in FireFox and Chrome.   In the console, you can type the variable name of your VertexComputer (let's call it vc) object to retrieve its internal details, you can also use `vc.getResultUnits()` to get all the unit objects to the console (your browser lets you navigate the object easily to allow you to dig deep into the data if needed).  
