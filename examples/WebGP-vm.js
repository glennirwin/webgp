...
void main() {
    ...
    vec2 vpmouse = (inverse(i_projection) * vec4(u_mouse.xy / u_resolution * 2.0 - 1.0,0.0,1.0)).xy;  // convert mouse to viewport coord
        ...
        function loop() {   // The render loop
            ...
            if (clicks == 3 && checkStack == false) {
              checkStack = true;
                ...
                stats.innerText = watch.cps().toFixed(1)+"fps\n"+watch.mpc().toFixed(3)+"ms";  watch.reset();
        ...
        loop();
          ...
          let cfg = config || { viewport: [Math.random()-0.5, Math.random()-0.5, 400/GP.canvas.clientWidth, 300/GP.canvas.clientHeight] };
      ...
      function handleFileSelect(evt) {  // for dragged and dropped files
         ...
         for (var i = 0, f; f = files[i]; i++) {
           if (f.type.match('text.*')) {
             ...
             console.error("I do not yet know how to handle file "+escape(f.name), ' (', f.type || 'n/a', ') - ', f.size, ' bytes, last modified: ',f.lastModifiedDate ? f.lastModifiedDate.toLocaleDateString() : 'n/a');
           }
         }
       }
