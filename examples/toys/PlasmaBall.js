...
Kit.PlasmaBall = {
 ...
 renderStep: {
        ...
        float noise( in float x ){return textureLod(u_noise, vec2(x*.01,1.),0.0).x;}
        ...
        float noise(in vec3 p)
        {
        	...
        	vec2 rg = textureLod( u_noise, (uv+ 0.5)/256.0, 0.0 ).yx;
        ...
        void main( )
        {
          vec2 p = ((inverse(u_projection) * vec4(gl_FragCoord.xy/u_resolution * 2.0 - 1.0,0.0,1.0)) ).xy * 0.5;
        	...
        	if (col.x > 0.0 && col.z > 0.0 && col.y > 0.0) fragColor = vec4(col*1.3, 1.0);
        } `
}
}
