...
float text(vec2 uv)
{
    ...
    float hour = floor(u_date.w/60.0/60.0);
    float minute = floor(mod(u_date.w/60.0,60.0));
    float second = floor(mod(u_date.w,60.0));
...
//    col += print_integer(float(u_frame),4,uv);
    ...
    TEXT_MODE = UNDERLINE;
    col += print_integer(u_date.z,2,uv);
    col += char(ch_lsl,uv);
    col += print_integer(u_date.y+1.0,2,uv);
    col += char(ch_lsl,uv);
    col += print_integer(u_date.x,4,uv);
    ...
    TEXT_MODE = UNDERLINE;
    col += print_integer(u_resolution.x,0,uv);
    col += char(ch_x,uv);
    col += print_integer(u_resolution.y,0,uv);
...
void main() {
    ...
    res = vec2(u_viewport.z * 2.0 * u_resolution.x, u_viewport.w * 2.0 * u_resolution.y) / DOWN_SCALE;
    vec2 duv = floor(uv * res);
  ...
  //  col *= (1.-distance(mod(uv,vec2(1.0)),vec2(0.65)))*1.2;
    col *= mix(vec3(0.09),vec3(0,1,0),pixel);

    if (col.r > 0.1 || col.g > 0.1 || col.b > 0.1) fragColor = vec4(col,0.7);
