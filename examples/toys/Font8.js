...
Kit.Font8 = {
 ...
 renderStep: {
        fragment: ` // Origin: https://www.shadertoy.com/view/XtfyRS

       float GlyphSDF( vec2 p, float glyph )
       {
           p = abs( p.x - .5 ) > .5 || abs( p.y - .5 ) > .5 ? vec2( 0. ) : p;
           return 2. * ( texture( u_iChannel0, vec2(1.,-1.)* p / 16. + fract( vec2(1.,-1.)*vec2( glyph, 15. - floor( glyph / 16.  ) ) / 16. ) ).w - 127. / 255. );
       }
       ...
       void main()
       {
         vec2 uv =  (((inverse(u_projection) * vec4((gl_FragCoord.xy/u_resolution.xy * 2.0 - 1.0),0.0,1.0))).xy + 1.0) / 2.0;  // for full quad (testing)
