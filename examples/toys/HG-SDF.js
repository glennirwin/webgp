 ...
 renderStep: {
    ...
    vec3 shade(vec3 ray_start, vec3 ray_dir, vec3 light_dir, vec4 hit)
    {
       ...
       if (hit.w == 0.0) {
          ...
          vec3 base_color = checker_texture(hit.xyz,ray_len/(u_resolution.y*u_viewport.w)).xyz;
    ...
    void main()
    {
//       vec2 uv = (gl_FragCoord.xy - u_resolution.xy*0.5) / u_esolution.y;
        vec2 uv = ((inverse(u_projection) * vec4(gl_FragCoord.xy/u_resolution * 2.0 - 1.0,0.0,1.0)) ).xy;
    ...
    float fField(vec3 p)
    {
       ...
       float sphere = length(p-vec3(1.+sin(u_time*.25)*.2,.8,1))-1.;
       float d;
       float r = 0.3;
       float n = 4.;
