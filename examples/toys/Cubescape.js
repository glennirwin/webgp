 ...
 renderStep: {
        ...
        vec3 render( in vec3 ro, in vec3 rd )
        {
            ...
            if( res.y > -0.5 )
            {
                ...
                vec3 ff = texcube( u_iChannel1, 0.1*vec3(pos.x,4.0*res.z-pos.y,pos.z), nor ).xyz;
                col *= ff.x;
        ...
        void main()
        {
        	freqs[0] = texture( u_iChannel0, vec2( 0.01, 0.25 ) ).x;
        	freqs[1] = texture( u_iChannel0, vec2( 0.07, 0.25 ) ).x;
            ...
            for( int i=0; i<ANTIALIAS; i++ )
            {
                ...
                vec2 xy = ((inverse(u_projection) * vec4(gl_FragCoord.xy/u_resolution * 2.0 - 1.0,0.0,1.0)) ).xy;
            ...
            // vigneting
        	vec2 q = (((inverse(u_projection) * vec4(gl_FragCoord.xy/u_resolution * 2.0 - 1.0,0.0,1.0)) ).xy+1.0)/2.0;   //fragCoord.xy/iResolution.xy;
