...
Kit.Terrain = {
 ...
 renderStep: {
        ...
        float noise1( in vec3 x )
        {
        	...
        	vec2 rg = textureLod( u_iChannel2, (uv+ 0.5)/256.0, 0.0 ).yx;
        #else
        	vec2 uv = (p.xy+vec2(37.0,17.0)*p.z);
        	vec2 rg1 = textureLod( u_iChannel2, (uv+ vec2(0.5,0.5))/256.0, 0.0 ).yx;
        	vec2 rg2 = textureLod( u_iChannel2, (uv+ vec2(1.5,0.5))/256.0, 0.0 ).yx;
        	vec2 rg3 = textureLod( u_iChannel2, (uv+ vec2(0.5,1.5))/256.0, 0.0 ).yx;
        	vec2 rg4 = textureLod( u_iChannel2, (uv+ vec2(1.5,1.5))/256.0, 0.0 ).yx;
        ...
        {
        	float th = smoothstep( 0.0, 0.7, textureLod( u_iChannel0, 0.001*q, 0.0 ).x );
            float rr = smoothstep( 0.1, 0.5, textureLod( u_iChannel1, 2.0*0.03*q, 0.0 ).y );
        	float h = 1.9;
        	#ifndef LOWDETAIL
        	h += -0.15 + (1.0-0.6*rr)*(1.5-1.0*th) * 0.3*(1.0-textureLod( u_iChannel0, 0.04*q*vec2(1.2,0.5), 0.0 ).x);
        	#endif
        	h += th*7.0;
            h += 0.3*rr;
            return -h;
        ...
        {
        	float th = smoothstep( 0.0, 0.7, textureGood( u_iChannel0, 0.001*q, 0.0 ).x );
            float rr = smoothstep( 0.1, 0.5, textureGood( u_iChannel1, 2.0*0.03*q, 0.0 ).y );
        	float h = 1.9;
        	h += th*7.0;
            return -h;
        }
        ...
        void main()
        {
        	...
        	float an = 0.5*(u_time-5.0);// + 12.0*(m.x-0.5);
        	...
        	{
                vec3 spos = ro + pt*rd;
                float clo = texture( u_iChannel0, 0.00006*spos.xz ).x;
            ...
            if( tmat.x<tmax )
            {
        		...
        		//if( tmat.z<0.5 )
        		{
        			...
        			float be = 1.0/1024.0;
        			float bf = 0.4;
        			bnor.x = texcube( u_iChannel0, bf*uvw+vec3(be,0.0,0.0), nor ).x - texcube( u_iChannel0, bf*uvw-vec3(be,0.0,0.0), nor ).x;
        			bnor.y = texcube( u_iChannel0, bf*uvw+vec3(0.0,be,0.0), nor ).x - texcube( u_iChannel0, bf*uvw-vec3(0.0,be,0.0), nor ).x;
        			bnor.z = texcube( u_iChannel0, bf*uvw+vec3(0.0,0.0,be), nor ).x - texcube( u_iChannel0, bf*uvw-vec3(0.0,0.0,be), nor ).x;
        			bnor = normalize(bnor);
        			float amo = 0.2  + 0.25*(1.0-smoothstep(0.6,0.7,nor.y) );
        			nor = normalize( nor + amo*(bnor-nor*dot(bnor,nor)) );

        			vec3 te = texcube( u_iChannel0, 0.15*uvw, nor ).xyz;
        			te = 0.05 + te;
        			mate.xyz = 0.6*te;
        			mate.w = 1.5*(0.5+0.5*te.x);
        			float th = smoothstep( 0.1, 0.4, texcube( u_iChannel0, 0.002*uvw, nor ).x );
        			...
        			mate.xyz *= 0.5;
        			float rr = smoothstep( 0.2, 0.4, texcube( u_iChannel1, 2.0*0.02*uvw, nor ).y );
        			mate.xyz *= mix( vec3(1.0), 1.5*vec3(0.25,0.24,0.22)*1.5, rr );
        			mate.xyz *= 1.5*pow(texcube( u_iChannel3, 8.0*uvw, nor ).xyz,vec3(0.5));
